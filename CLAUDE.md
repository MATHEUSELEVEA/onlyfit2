...
2. **Nunca tamanho de fonte arbitrário** (`text-[13px]`) nem `font-bold` solto. Só tokens (`text-body`, `text-label`…). Tipografia é global e igual nos 2 temas.
...
7. **Cliente nunca escreve em tabela de pagamento.** O front só escreve interações do próprio usuário (`post_likes`, `post_comments`, `creator_follows`). `subscriptions`/`payment_subscriptions` são somente leitura — assinar é checkout no servidor. Ver `docs/DATABASE.md`.
...
docs/temas/     specs de cor dos 2 temas (fonte da verdade do design)
...
- **Abriu branch + PR e mergeou na `main` — nunca commit direto na `main`** (regra 9). Ver `docs/GIT-FLOW.md`.
