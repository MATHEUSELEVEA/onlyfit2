...
export function RoutinePage() {
  ...
  return (
    <>
              ...
              {routines.map((routine) => (
                <button key={routine.id} type="button" onClick={() => setSelectedId(routine.id)} className="flex min-h-[152px] flex-col items-start justify-between rounded-2xl border border-outline-variant/40 bg-surface-container p-4 text-left transition-transform active:scale-[0.97]">
                  ...
                  <span>
                    ...
                    <span className="mt-1 block font-sans text-body-sm text-on-surface-variant">{routine.checked.length}/{routine.times.length} {t('meufit.routine.done')}</span>
                  </span>
                </button>
              ))}
            </div>
      ...
      {wizardOpen && <RoutineWizard onClose={() => setWizardOpen(false)} onCreate={(routine) => { setRoutines((current) => [...current, routine]); setWizardOpen(false); }} />}

      <BottomSheet open={Boolean(selected)} onClose={() => setSelectedId(null)} title={selected?.title ?? ''}>
        {selected && <RoutineDetail routine={selected} onToggle={toggleCheck} />}
      </BottomSheet>
    </>
  );
}

function RoutineWizard({ onClose, onCreate }: { onClose: () => void; onCreate: (routine: Routine) => void }) {
  const { t } = useTranslation();
  ...
  const schedule = timingMode === 'specific' ? times : intervalTimes(start, end, everyHours);
