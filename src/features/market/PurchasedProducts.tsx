...
// Corpo da aba "Minhas compras": histórico do que o usuário já adquiriu.
// Sem barra de topo própria — a casca (ProductsPage) cuida disso. `onBrowse`
// leva o usuário de volta à vitrine quando a coleção está vazia.
export function PurchasedProducts({ onBrowse }: { onBrowse: () => void }) {
    ...
    <>
        ...
        <div className="bg-background px-4 pb-3 pt-4">
          <div className="relative">
            ...
            <input
              ...
              aria-label="Buscar nas minhas compras"
      ...
      {!isLoading && !isError && products.length === 0 && (
        <div className="flex flex-col items-center px-7 py-16 text-center">
          ...
          <p className="mt-1 max-w-sm font-sans text-body text-on-surface-variant">
            Os treinos, ebooks, dietas e outros produtos que você comprar aparecerão aqui.
          </p>
          <button
            type="button"
            onClick={onBrowse}
      ...
      {!isLoading && !isError && products.length > 0 && visible.length === 0 && (
        <div className="px-6 py-14 text-center">
          <p className="font-sans text-title text-on-surface">Nenhuma compra encontrada</p>
