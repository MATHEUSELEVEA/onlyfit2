...
## 0012 — Ordem da grade do perfil em `posts.metadata`
- Data: 2026-07-20
- Status: aceita
- Contexto: o creator precisa organizar visualmente a grade do próprio perfil sem alterar a ordem cronológica do feed e sem adicionar uma coluna ao schema compartilhado com o v1/desktop.
- Decisão: o mobile persiste `profile_grid_position` no `metadata` de cada post. A aba Feed do perfil ordena por esse valor; posts novos, ainda sem posição manual, aparecem primeiro. O feed principal continua usando exclusivamente sua ordenação cronológica/recomendada.
- Consequência: a organização é persistente e compatível com clientes antigos, que simplesmente ignoram a chave. Como a gravação atual atualiza cada post sob a RLS existente, uma falha parcial é reconciliada pelo refetch e pode exigir que o usuário salve a ordem novamente.
