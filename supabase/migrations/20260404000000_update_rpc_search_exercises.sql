-- Re-create rpc_search_exercises to include instructions and equipment text
...
AS $$
  WITH base AS (
    SELECT
      ...
      el.category,
      el.equipment,
      CASE WHEN lower(locale) IN ('pt','ptbr','pt-br') THEN el.instructions_ptbr ELSE el.instructions_en END AS instructions,
      el.primary_muscles,
      el.thumb_asset_id,
      el.video_url,
      el.thumb_url,
  ...
  SELECT r.id, r.name, r.category, r.equipment, r.instructions, r.primary_muscles, r.thumb_asset_id, r.video_url, r.thumb_url, r.rank
  FROM ranked r
